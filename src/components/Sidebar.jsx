...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    { icon: Calendar, label: 'Section', path: '/section' },
      ...
      <aside className={cn(
        ...
        <div className="p-6 flex items-center justify-between">
          <Link to="/dashboard" className="flex items-center gap-2" onClick={onClose}>
            ...
            <span className="text-lg font-bold text-gray-900">Scan And Know</span>
          </Link>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon size={20} className={cn(isActive ? "text-indigo-600" : "text-gray-400 group-hover:text-gray-600")} />
                {item.label}
              </Link>
            );
          })}
